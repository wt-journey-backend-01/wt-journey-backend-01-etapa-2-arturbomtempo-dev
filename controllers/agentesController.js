...
const { agenteSchema } = require('../utils/validationSchemas');
...
const getAgenteById = (req, res, next) => {
  ...
  if (!agente) return next({ message: 'Agente não encontrado', statusCode: 404 });
...
const updateAgente = (req, res, next) => {
  try {
    ...
    if (!atualizado) return next({ message: 'Agente não encontrado', statusCode: 404 });
